  ...
  switch (process_state)
  {
    ...
    case DXL_PROCESS_INST:
      ...
      if (dxl_ret == DXL_RET_RX_INST)
      {
        ...
        {
          Serial2.println("DXL_PROCESS_BROAD_READ");
          pre_time = micros();
    ...
    case DXL_PROCESS_BROAD_READ:
      dxl_ret = dxlRxPacket3(&dxl_sp, data3, data3len);
      ...
      else
      {
        ...
        {
          process_state = DXL_PROCESS_INST;
          Serial2.println(" Bulk Read timeout");
        }
      }
      break;
...
dxl_error_t bulk_read(dxl_t *p_dxl)
{
  ...
  Serial2.print(" bulk param count: ");
  Serial2.println(rx_id_cnt);

  if (p_dxl->rx.param_length < 5 || (p_dxl->rx.param_length%5) != 0)
  {
    Serial2.println(" DXL_RET_ERROR_LENGTH ");
    return DXL_RET_ERROR_LENGTH;
  }
  ...
  for (i=0; i<rx_id_cnt; i++)
  {
    ...
    Serial2.print(" bulk addr : ");
    Serial2.println(addr);
    Serial2.print(" bulk length : ");
    Serial2.println(length);
    
    Serial2.print(" bulk in id : ");
    Serial2.println(p_data[0]);

    if (p_data[0] == p_dxl->id)
    {
      p_dxl->current_id = p_dxl->id;
      Serial2.println(" bulk found id ! ");
      break;
    }
  ...
  if (p_dxl->current_id == p_dxl->id)
  {
    ...
    else
    {
      ...
      if (ret == DXL_RET_OK)
      {
        Serial2.println(" DXL_RET_PROCESS_BROAD_READ ");
        ret = DXL_RET_PROCESS_BROAD_READ;
      }
    }
  }
